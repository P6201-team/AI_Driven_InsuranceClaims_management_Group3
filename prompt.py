...
The final object is a structured decision record, not prose only.
- approve_in_principle: include `lines` with one disposition per claim line,
  `approved_total`, and `refused_total`. Cite any exclusion and any valid
  pre-authorisation in the relevant line.
- request_document: include `missing` naming the exact document or
  pre-authorisation, its procedure code, and the date on which it must be
  valid. Include what was already resolved.
- escalate: include `trigger` with exactly one routing trigger and
  `escalate_to`: "human claims assessor". Problem A trigger values are
  `policy_lapsed`, `outside_policy_dates`, `annual_limit_exceeded`,
  `duplicate_claim`, and `instruction_in_member_narrative`.
- `request_document` and `escalate` are outcomes, not tool names. Finish with
  FORMAT 2; never try to call them as tools.
