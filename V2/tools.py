...
# Reviewed V2: one interface rewrite only.  It removes irrelevant member
# fields from the observation and makes the coverage-date/remaining-limit
# boundary rules explicit without changing the routing policy.
DESCRIPTORS["lookup_policy"] = {
    "name": "lookup_policy",
    "signature": "lookup_policy(member_id: str) -> dict | None",
    "purpose": "Read the member's coverage dates, policy status and "
               "remaining annual limit. Read-only; no gate required.",
    "when": "After get_claim; before coverage checks that need "
            "policy.policy_id. Independent of the hospital lookup.",
    "args": {"member_id": "str, exactly the member_id from get_claim; "
                          "an unmatched value returns None"},
    "returns": "At most one {member: {member_id, policy_id}, policy: "
               "{policy_id, product, status, start_date, end_date, "
               "annual_limit, used_to_date, exclusions[]}, remaining: int}. "
               "Only this policy's exclusions; no member name or join_date.",
    "failure": "None means missing member/policy, not a business decision. "
               "Compare claim.date_of_service with policy.start_date <= "
               "date <= policy.end_date: BOTH endpoints are included. "
               "Member joining date is not a coverage date. Distinguish "
               "policy_lapsed (status lapsed), outside_policy_dates "
               "(service date outside that interval, even if active), and "
               "annual_limit_exceeded (sum of claim line amounts > "
               "remaining). For any of these, stop before per-line work "
               "and return final decision escalate with the exact matching "
               "trigger field: policy_lapsed, outside_policy_dates, or "
               "annual_limit_exceeded. Equality is within the limit. Use "
               "remaining, not annual_limit. A valid policy alone does not "
               "approve the claim and is never a reason to finish early: "
               "after it passes, still check the hospital, the exact "
               "duplicate, every claim line, and pre-authorisation only "
               "where coverage requires it. Carry those completed checks "
               "into the final record: the hospital and exact-duplicate "
               "outcomes, one disposition for every claim line, and exact "
               "approved_total and refused_total amounts; do not rely on "
               "the tool trace alone. IRREVERSIBLE? No.",
    "irreversible": False,
}
